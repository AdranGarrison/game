...
#TODO: Severe wounds left to heal without treatment should have a chance to be left (near-)permanently deformed.
#TODO: Ductile materials need damage resolution. Need to figure out how to handle denting.
...
class Limb(Widget):
    ...
    def damageresolve(self,attack,attacker,reactionforce=False):
        ...
        if self.armor is not None:
            ...
            if self.armor.coverage*random.gauss(self.stats['luc'],1)>random.random()*(attacker.stats['luc']+2*attacker.stats['tec'])/3+attack.arpen:
                ...
                if attack.contact==False and self.armor is not None:
                    ...
                    area=area+(3.54*self.armor.thickness*area**0.5+3.14*self.armor.thickness**2)*self.armor.dissipationfactor
                    pressure=f/area
                    attack.force=f
        ...
        while i>=0:
            ...
            if attack.contact==False:
                ...
                area=area+(3.54*self.layers[i].thickness*area**0.5+3.14*self.layers[i].thickness**2)*self.layers[i].dissipationfactor/softnessfactor
                pressure=f/area
                attack.force=f
                attack.pressure=pressure
            i-=1
    ...
    def unequip(self,slot,cascade=False,destroyed=False,drop=False):
        ...
        if self.equipment[slot] is not None:
            if cascade==False:
                if destroyed==False:
                    if drop==True:
                        dropitem(self.equipment[slot])
                    else:
                        inventoryadd(self.equipment[slot])
    ...
    def sever(self, primary=True):
        if primary:
            ...
            self.location[0]=min(max(self.owner.location[0]+int(random.gauss(0,2)),0),50)
            self.location[1]=min(max(self.owner.location[1]+int(random.gauss(0,2)),0),50)
...
#TODO: Blocking still requires implementation.
...
class Creature(Widget):
    ...
    def on_turn(self,turns=1):
        ...
        #collapse from exhaustion if stamina runs too low
            if self.stamina[0]<self.stats['str']*random.gauss(4/self.stats['luc']**0.5,1/self.stats['luc']):
                ...
                self.stamina[0]+=int(self.stats['str']**0.8)
        ...
        #handle regeneration of stamina and focus, if applicable
            ...
            if self.attacked==False:
                self.stamina[0]+=int(self.stats['str']**0.8-self.movemass/self.stats['str']**1.5+1)
        ...
        #If alive, recover, and test for walking and grasping
            self.can_walk=False
            self.can_grasp=False
            self.balance=0
            for i in self.limbs:
    ...
    def equip(self,item):
        for i in self.limbs:
            if item.wield in i.equipment:
                if i.equipment[item.wield]==None:
                    i.equip(item)
                    if item.wield=='grasp':
    ...
    def attack(self,target):
        ...
        while endattack==False:
            ...
            while hit_location.ability<=0 and hit_location.contains_vitals==False:
                if random.random()*self.stats['tec']>random.random()*target.stats['luc']:
                    break
                hit_location=A.targetchoice(target)
            ...
            if self.stats['tec']/attacksmade>random.triangular()*90/(self.stats['luc'])**0.3:
                atk.append(random.choice(self.attacks))
    ...
    def evasion(self,attack):
        ...
        for i in self.conditions:
            if incapacitate.search(i):
                attack.pressure=2*attack.pressure*attack.attacker.stats['tec']
                attack.force=2*attack.force*attack.attacker.stats['tec']
                attack.arpen+=0.5
        ...
        if attack.time<reactiontime:
            attack.pressure=attack.pressure*attack.attacker.stats['tec']**0.5
            attack.force=attack.force*attack.attacker.stats['tec']**0.5
            messages.append('The attack takes {} completely unaware!'.format(self.name))
            return


        dodgetime=reactiontime+((self.focus[1]/(1+self.focus[0]))**0.5)*random.gauss(3/(self.stats['tec']*self.stats['luc']**0.5),.04)*(self.movemass/(self.stats['str']*self.stamina[0]/(self.stamina[1]+1)))*3*attack.accuracy/(self.balance+0.01)
        if dodgetime<attack.time and random.random()*attack.attacker.stats['tec']/2<random.random()*self.stats['tec']:
            attack.dodged=True
        ...
        for i in self.equipped_items:
            if i.parry==True:
                parrytime=reactiontime+attack.accuracy*((self.focus[1]/(self.focus[0]+1))**0.5)*random.gauss(6/(self.stats['tec']*self.stats['luc']**0.5),.04)*(30*i.I+i.equipped.mass)/max(i.equipped.stats['str']*i.length*i.equipped.ability,0.001)
                if parrytime<attack.time and random.random()*attack.attacker.stats['tec']/2<random.random()*self.stats['tec']:
                    print(i,attack.time,reactiontime,parrytime)
                    attack.parried=True
                    attack.target=i
                    attack.basetarget=i
                    return
...
#Cutting---------------------------------------------------------------------
            ...
            if isinstance(attack.basetarget,Limb):
                shearforce=min(shearforce*(damagedobject.mass/basetarget.movemass)**0.5,shearforce)
            if shearforce>self.shear_strength*1000000 and attack.contact==True and attack.type=='cut':
                severity=abs(random.gauss(((attack.force/(200+attack.force))*0.1/(damagedobject.thickness+0.1))
                                          *(attacker.stats['tec']+0.2*attacker.stats['luc'])/(defenderstats['luc']
                                            +0.2*defenderstats['tec']),1))
                damagedobject.damage['cut']=(damagedobject.damage['cut']**2+severity**2)**0.5
                if damagedobject.damage['cut']>=1: contact=True
                bruisable=False
                crushable=False
#----------------------------------------------------------------------------

#Crushing--------------------------------------------------------------------
            if attack.force>self.tensile_strength*(1200000-200000*random.triangular(low=0,high=1,mode=m))*damagedobject.length*damagedobject.radius and damagedobject.damage['crush']==0 and attack.contact==True and crushable==True:
...
#Cracking, Breaking, and Shattering------------------------------------------
            hitloc=random.triangular(low=0.00001,high=1,mode=m)*damagedobject.length
            crackforce=min(attack.force*(damagedobject.mass/basetarget.movemass)**0.5,attack.force)
...
#Bruising
            if attack.pressure>self.tensile_strength*230000 and bruisable==True:
                severity=(attack.pressure/(self.tensile_strength*300000)-1)*random.gauss(0.5*attacker.stats['luc']/defenderstats['luc'],0.5)
...
class Item():
    ...
    def damageresolve(self,attack,attacker,reactionforce=False):
        ...
        if self.damage['crush']>self.olddamage['crush']:
            ...
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*300*self.damage['crush']/attack.basetarget.owner.stats['wil']**0.5
            self.functioncheck()
            return
        ...
        if self.damage['cut']>self.olddamage['cut']:
            ...
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*40*self.damage['cut']/attack.basetarget.owner.stats['wil']**0.5
        ...
        if self.damage['bruise']>self.olddamage['bruise']:
            ...
            attack.damage_dealt+=1
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*5*min(self.damage['bruise'],10)/attack.basetarget.owner.stats['wil']**0.5
        ...
        if self.damage['shatter']>self.olddamage['shatter']:
            ...
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*250*self.damage['shatter']/attack.basetarget.owner.stats['wil']**0.5
            self.functioncheck()
            return
        ...
        if self.damage['break']>self.olddamage['break']:
            ...
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*175*self.damage['break']/attack.basetarget.owner.stats['wil']**0.5
            self.functioncheck()
            return
        ...
        if self.damage['crack']>self.olddamage['crack'] and self.olddamage['crack']==0:
            ...
            if isinstance(attack.basetarget,Limb) and self in attack.basetarget.layers:
                attack.basetarget.owner.pain+=attack.basetarget.painfactor*100*self.damage['crack']/attack.basetarget.owner.stats['wil']**0.5+2
            self.functioncheck()
            return
    ...
    def functioncheck(self):
        if self.damage['shatter']+self.damage['crush']+self.damage['break']>=1:
            self.function=0
        else:
            self.function=1-self.damage['crack']-self.damage['bruise']/20-self.damage['cut']
